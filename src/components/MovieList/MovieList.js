...
const MovieList = () => {
  ...
  const getMovies = useMemo(async () => {
    try {
      ...
      const { data } = await movieService.getAllMovies(
        moviesCategories.moviesFor("popular", currentGenre, currentPage)
      );
      setFoundMovies(data);
      setIsLoading(false);
    } catch (e) {
  ...
  const filteredMovies =
    results && results.filter((movie) => movie.title.toLowerCase().includes(searchValue.toLowerCase()));

  return (
    <section className={css.movies}>
      <div className={app.container}>
        <div className={css.movies__inner}>
          <h3 className={`${css.movies__title} ${app.category}`}>All Movies</h3>
        ...
        <ul className={css.movies__items}>
          {isLoading
            ? results && filteredMovies.map((movie) => <Skeleton key={movie.id} amount={20} />)
            : results &&
              filteredMovies.map((movie) => (
                <MovieListCard key={movie.id} movie={movie} genres={genres.genres} styleCard={css.movies__item} />
              ))}
        </ul>
        <Pagination
          count={totalPages}
